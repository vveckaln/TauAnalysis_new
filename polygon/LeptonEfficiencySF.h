...
class LeptonEfficiencySF
{
 public:
  ...
  pair<float,float> getLeptonEfficiency(const float pt, const float input_eta, const int input_id, const string wp) const 
    {
      const float eta = fabs(input_eta);
      const uint id = abs(input_id);
      
      pair<float, float> eff(1.0, 0.04);
      switch(id){
      case 11:
	{
	  if(wp=="loose")
