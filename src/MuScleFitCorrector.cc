...
MuScleFitCorrector *getMuonCorrector(const TString baseDir, const TString url)
{
  const bool is7TeV(url.Contains("7TeV"));
  const bool isData(url.Contains("Data"));
  const bool is2012D(url.Contains("2012D"));
  TString fName("");
  if(is7TeV) return 0;
